...
import { workCaseMap, workCases } from "@/data/portfolio.weverse";
...
export async function generateMetadata({ params }: WorkPageProps): Promise<Metadata> {
  ...
  if (!project) {
    return {
      title: "Weverse 서비스기획 지원용 포트폴리오",
    };
  }

  return {
    title: `${project.title} / Weverse 서비스기획 지원용 포트폴리오`,
...
export default async function WeverseWorkDetailPage({ params }: WorkPageProps) {
  const { slug } = await params;
  const project = workCaseMap[slug];
