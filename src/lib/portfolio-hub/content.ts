...
export const resumeProfile = {
  ...
  subline:
    "SFTI-CMU의 감정 콘텐츠·시각 정체성 프레임을 AHEYA 서비스/마케팅 자산과 Loom IP 콘텐츠로 확장한 AI Creative 포트폴리오입니다.",
  profile:
    "브랜드·제품·IP 아이디어를 숏폼 영상, 이미지/음악 콘텐츠, 콘텐츠형 서비스 프로토타입으로 구현해왔습니다. 무신사 AI 광고제 출품작, AI 아이돌 IP 서비스 프로토타입 Loom, ADSB AI 숏폼 크리에이티브를 진행했고, SFTI-CMU에서는 감정 콘텐츠와 시각 정체성을 영문 초록/포스터형 자료로 구체화했습니다. AHEYA에서는 이 관점을 서비스 첫인상과 마케팅용 이미지·영상 자산에 적용하며 브리프 해석, 메시지 구조화, 키비주얼/스토리보드 설계, AI 생성 결과 검수, 컷 편집 흐름 조정, 사용자 행동 기준 설정까지 연결했습니다. 생성형 AI를 단순 제작 도구로 쓰는 것보다, 기획 의도를 실제 공개 가능한 콘텐츠와 서비스 경험으로 만드는 데 강점이 있습니다.",
  competencies: [
