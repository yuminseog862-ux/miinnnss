...
export const abLunaSlides = [
  ...
  },
  {
    ...
    accent: "violet",
    variant: "matrix",
    intent: "AB-Luna의 문제를 'AI 툴 부족'이 아니라 result-to-state conversion failure로 좁힌다.",
    claim:
      "학생 팀은 여러 AI 툴을 쓰지만 결과물이 현재 상태, 다음 액션, 안정적인 handoff로 자동 전환되지 않는 문제",
  ...
  },
  {
    ...
    table: {
      ...
      rows: [
        ...
        ["진입 전략", "non-coding user first, free-tier-friendly adoption, GitHub-backed bridge", "power-user repo workflow에만 갇히는 구조"],
      ],
    },
    note:
  ...
  },
  {
    no: 42,
    section: "AB-Luna",
    title: "KPI Design & Readiness",
    label: "KPI System",
    accent: "cyan",
    variant: "matrix",
    intent: "AB-Luna가 아이디어에 머물지 않고 측정 가능한 제품 흐름으로 설계됐음을 보여준다.",
    claim:
      "가입자 수만 보는 대신 signup, verification, first project, state view, collaboration, publish, repeat까지 이어지는 KPI 흐름",
    ...
    table: {
      ...
      rows: [
        ...
        ["Readiness", "product KPI는 대부분 ready, reporting surface와 growth/revenue KPI는 gap", "아이디어가 아니라 측정 가능한 운영 과제로 정리"],
      ],
    },
    note:
  ...
  },
  {
    ...
    claim:
      "새 AI 모델을 붙이는 것보다 여러 AI output 사이의 상태 단절을 읽히는 workflow로 바꾸는 제품 문제 정의",
    include: [
  ...
  },
  {
    ...
    claim:
      "Google Form 기반으로 문제 공감, 현재 workflow, handoff pain, MVP 반응을 수집할 수 있는 검증면",
    include: ["Google Form", "User research setup", "Problem validation", "MVP response"],
    slots: [
    ...
    table: {
      ...
      rows: [
        ...
        ["문제 공감", "산출물이 흩어진 뒤 현재 상태와 다음 액션이 흐려지는가", "result-to-state conversion failure의 강도 확인"],
    ...
    links: [
      {
        label: "AB-Luna Google Form",
        href: "https://docs.google.com/forms/d/13FriqN3dPy3Nq3ooovKF_C1rCPtwhJLEtCaDaWdGXm0/edit",
        description: "AB-Luna 문제 공감과 MVP 반응을 수집하기 위한 Google Form 원본",
      },
    ],
    note:
      "폼 응답은 problem framing, MVP copy, handoff flow 우선순위를 조정하는 리서치 입력으로 사용한다.",
  },
] satisfies Slide[];
