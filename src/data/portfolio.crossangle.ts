...
export const adsbSupportingProof = {
  ...
  body:
    "Andersson Bell 산학협력 프로젝트에서 브랜드 무드와 타깃 감성을 분석해 숏폼용 이미지·영상 콘셉트로 전환했습니다. AI 이미지·영상 툴로 여러 시각 방향을 테스트하고, 반응 가능성이 높은 표현 방식으로 정리했습니다.",
  points: [
    "범위: 브랜드 무드 분석 / 타깃 감성 해석 / 숏폼 콘셉트 기획",
    "흐름: 리서치 → 무드보드 → 스토리보드 → 이미지·영상 테스트 → 표현 방식 정리",
    "판단: 톤이 분산되는 콜라주형 시도를 줄이고, 브랜드 인상이 빨리 읽히는 장면 중심으로 수렴",
    "JD 연결: X/숏폼 콘텐츠 운영과 미디어 업데이트를 위한 시각 자산 기획 보조 역량",
  ],
...
export const sftiSupportingProof = {
  ...
  points: [
    "범위: 브랜드 정체성 / 생성형 AI 시각 콘텐츠 / 문서 구조화",
    "흐름: 자료 조사 → 논지 구성 → 초안 작성 → 피드백 반영 → 연구 문서 정리",
    "판단: 복잡한 주제를 읽히는 글 구조로 바꾸는 리서치 writing 경험",
    "JD 연결: 리서치 내용을 외부 독자가 이해할 수 있는 문서와 콘텐츠 구조로 압축하는 보조 역량",
  ],
